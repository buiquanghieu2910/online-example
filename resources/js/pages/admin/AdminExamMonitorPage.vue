...
async function fetchData({ showErrorToast = true, showLoading = false } = {}) {
    ...
    try {
        const { data } = await api.get('/admin/monitor/active-attempts', { params: buildParams() });
...
async function openTimeline(item) {
    ...
    try {
        const { data } = await api.get(`/admin/monitor/active-attempts/${item.user_exam_id}/timeline`);
