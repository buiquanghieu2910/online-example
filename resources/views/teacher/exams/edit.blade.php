...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
    ...
    <form action="{{ route('teacher.exams.update', $exam) }}" method="POST" onsubmit="return confirmUpdate(this, 'Bạn có chắc chắn muốn cập nhật bài tập này?');">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4"
                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-blue-500 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $exam->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('teacher.exams.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded">
                Hủy
            </a>
