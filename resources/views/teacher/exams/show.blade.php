...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
    ...
    <div class="flex space-x-3">
        <a href="{{ route('teacher.questions.index', $exam) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
            Quản lý câu hỏi
        </a>
        <a href="{{ route('teacher.exams.assign', $exam) }}" class="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded">
            Gán người dùng
        </a>
        <a href="{{ route('teacher.exams.edit', $exam) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Chỉnh sửa
        </a>
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
                    ...
                    @foreach($exam->assignedUsers as $user)
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                ...
                                @if($totalAttempts > 1)
                                    <a href="{{ route('teacher.exams.history', [$exam, $user]) }}" 
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex items-center space-x-3">
                                    @if($userExam && $userExam->status === 'completed')
                                        <form action="{{ route('teacher.results.reset', $userExam) }}" method="POST" class="inline" onsubmit="return confirmUpdate(this, 'Bạn có chắc chắn muốn cho người dùng này làm lại bài thi?');">
                                            @csrf
                                            <button type="submit" 
                                    ...
                                    @endif
                                    <form action="{{ route('teacher.exams.unassign', [$exam, $user]) }}" method="POST" class="inline" onsubmit="return confirmDelete(this, 'Bạn có chắc chắn muốn gỡ người dùng này khỏi bài tập?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
    ...
    @else
        <p class="text-gray-500 dark:text-gray-400">Chưa có người dùng nào được gán. <a href="{{ route('teacher.exams.assign', $exam) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400">Gán ngay</a></p>
    @endif
</div>
