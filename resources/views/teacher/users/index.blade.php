...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow transition-colors">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <a href="{{ route('teacher.students.create') }}" class="bg-blue-500 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition-colors">
            Tạo người dùng mới
        </a>
    </div>
                    ...
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('teacher.students.show', $user) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mr-3">Xem</a>
                                <a href="{{ route('teacher.students.edit', $user) }}" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-900 dark:hover:text-yellow-300 mr-3">Sửa</a>
                                <form action="{{ route('teacher.students.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirmDelete(this, 'Bạn có chắc muốn xóa người dùng này?');">
                                    @csrf
                                    @method('DELETE')
        ...
        @forelse($users as $user)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-700 relative">
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-700 rounded-lg shadow-lg py-1 z-10 border border-gray-200 dark:border-gray-600">
                            <a href="{{ route('teacher.students.show', $user) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600">
                            ...
                            <a href="{{ route('teacher.students.edit', $user) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600">
                                <span class="flex items-center">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <form action="{{ route('teacher.students.destroy', $user) }}" method="POST" onsubmit="return confirmDelete(this, 'Bạn có chắc muốn xóa người dùng này?');">
                                @csrf
                                @method('DELETE')
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between">
                        <span class="text-sm text-gray-600 dark:text-gray-400">Số bài thi:</span>
                        <span class="text-sm text-gray-900 dark:text-white">{{ $user->userExams->count() }}</span>
                    </div>
                </div>
            </div>
        @empty
