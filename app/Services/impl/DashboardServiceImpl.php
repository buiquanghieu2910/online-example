...
class DashboardServiceImpl implements IDashboardService
{
    public function getOverview(User $user, array $filters = []): array
    {
        ...
        $examOptions = $this->examOptions($user, $filters['class_id'] ?? null);
        $allowedClassIds = $classOptions->pluck('id')->all();
        $allowedExamIds = $examOptions->pluck('id')->all();

        $classId = isset($filters['class_id']) && in_array((int) $filters['class_id'], $allowedClassIds, true)
            ? (int) $filters['class_id']
            : null;
        $examId = isset($filters['exam_id']) && in_array((int) $filters['exam_id'], $allowedExamIds, true)
            ? (int) $filters['exam_id']
            : null;
        ...
        return [
            ...
            'question_quality' => $this->buildQuestionQuality($user, $fromDate, $classId, $examId),
            'filters' => [
                'days' => $days,
                'class_id' => $classId,
                'exam_id' => $examId,
